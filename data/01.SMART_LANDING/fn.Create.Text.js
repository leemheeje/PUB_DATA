...
fn.Create['Text'] = {
	init: function (utills, obj) {
		...
		this.create_key = this.options.keys[2];
		this.obj = $.extend(true, {
			classNames: {
				modal: '#modalBoxCreateText',
				modalInFormSmartEdite: '#txtContents0',
				colorPicker: '.cmmColorPicker',
				submitButton: '.modalUploadTextButton',
				cmmSortableWrap: '.cmmSortableWrap.step3',
				dataTotalKey: 'data-total-key',
			},
			beforeCallback: function () {
				$('#modalBoxCreateText').on('hidden.bs.modal', function (e) {
					if(oEditors&&oEditors.length){
						oEditors.getById["txtContents0"].exec("SET_IR", ['']);
					}else{
						$('#txtContents0').val(''); //초기화
					}
					$('#modalBoxCreateText .cmmColorPicker').minicolors('value', '#fff');//초기화
				});
			},
			afterCallback: function () {
				$('#modalBoxCreateText').modal('hide'); //밖에서 컨트롤옮겨야함
				if(oEditors&&oEditors.length){
					oEditors.getById["txtContents0"].exec("SET_IR", ['']);
				}else{
					$('#txtContents0').val('');//초기화
